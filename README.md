...
## If the page loads text but then goes blank

This was an issue in an earlier version, now fixed. The network diagram used to load
its drawing library from an external CDN; some hosting environments block that call,
which could blank the page. The library now ships inside this repository
(`assets/vis-network.min.js`) and loads with no external network request. If the
diagram still fails to render for any reason, it now fails gracefully with a small
message instead of a blank page, and the same data remains visible in the tables
below it.
